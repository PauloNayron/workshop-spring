...
public class Instantiation implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        var post1 = Post.builder().date(LocalDate.parse("2019-06-01")).title("Partiu viagem").body("Vou viajar para São Paulo. Abraços!").author(new AuthorDTO(maria)).build();
        var post2 = Post.builder().date(LocalDate.parse("2021-06-01")).title("Bom dia").body("Acordei Feliz hoje!").author(new AuthorDTO(maria)).build();


        var c1 = new CommentDTO("Boa viagem!", LocalDate.parse("2019-06-01"), new AuthorDTO(alex));
        var c2 = new CommentDTO("Aproveite!", LocalDate.parse("2019-06-01"), new AuthorDTO(bob));
        var c3 = new CommentDTO("Tenha um ótimo dia!", LocalDate.parse("2021-06-01"), new AuthorDTO(alex));

        post1.setComments(Arrays.asList(c1, c2));
        post2.setComments(Collections.singletonList(c3));
